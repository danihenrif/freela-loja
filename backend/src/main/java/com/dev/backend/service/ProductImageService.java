...
public class ProductImageService {
    ...
    /*public List<ProductImage> getImageById(Long id) {
        Optional<ProductImage> listProductImage = productImageRepository.findById(id);
        
        for (ProductImage productImage : listProductImage) {
            try (InputStream in = new FileInputStream("c:/imagens/"+productImage.getName())) {
                productImage.setArquivo(IOUtils.toByteArray(in));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public ProductImage addProductImage(Long idProduct, MultipartFile file) {
        ...
        try {
            if (!file.isEmpty()) {
                ...
                Path path = Paths
                        .get("C:\\Users\\Daniel\\Documents\\github\\freela-loja\\backend\\src\\main\\resources\\" + imageName);
    ...
    /*public ProductImage updateProductImage(ProductImage productImage, Long id) throws NotFoundException {
        try {
